...
from trust_constr import minimize, NonlinearConstraint, LinearConstraint,\
                         Bounds, check_grad
...
@pytest.mark.parametrize("data,factorization_method,rtol", 
...
def test_generate_pareto_data(data, factorization_method, rtol):
    ...
    for i in range(len(response_surfaces)):
        test_points = uniform(low=[pair[0] for pair in bounds],
                              high=[pair[1] for pair in bounds],
                              size=(10,len(bounds)))
        for test_point in test_points:
            difference = check_grad(partial(evaluate_response_surface, terms, response_surfaces[i]), 
                                    partial(evaluate_response_surface_grad, terms, response_surfaces[i]),
                                    test_point)
            assert difference/evaluate_response_surface(terms, response_surfaces[i], test_point) < 1e-4
    ...
    for index,target in enumerate(output_targets):
        if target:
            constraints.append(
                NonlinearConstraint(partial(evaluate_response_surface, 
                                            terms, response_surfaces[index]), 
                                    target, target,
    ...
    with suppress_warnings() as sup:
        ...
        res = minimize(objective_func, x0, args=(x_axis_index,-1.0), bounds=bounds, 
    ...
    for x_value in pareto_points:
        ...
        res = minimize(objective_func, x0, args=(y_axis_index,y_axis_sign), bounds=bounds,
    ...
    for current_input in pareto_input_values:
        ...
        for i in range(outputs.shape[1]):
            current_output.append(evaluate_response_surface(terms, response_surfaces[i], 
                                                            current_input))
...
def test_no_constraints():
    res = minimize(rosenbrock_function, np.array([0.1, -0.5, -5.0]),
                   options={'disp': False})
